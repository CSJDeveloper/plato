...
def checkpoint_personalized_metrics(
    ...
    np.save(os.path.join(save_location, labels_filename), metrics_holder.labels.detach().cpu().numpy())
    np.save(
        os.path.join(save_location, predictions_filename), metrics_holder.predictions.detach().cpu().numpy()
    )

    if metrics_holder.encodings is not None:
        np.save(
            os.path.join(save_location, encodings_filename), metrics_holder.encodings.detach().cpu().numpy()
        )
